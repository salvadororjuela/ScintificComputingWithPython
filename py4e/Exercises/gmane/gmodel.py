"""This program initiates the cleaning process of the raw data produced
by gmane.py"""

import sqlite3
import time
import re
import zlib # Used to compress some of the data
...
# Take all the email addresses and put them into a list
allsenders = list()
...
# Go through all messages and get all senders, objects and guids and store them in a dictionary
...
for message_row in cur_1:
    ...
    # Apply the sender mapping. Look up sender, if it is in there, get the entry key, otherwise, get sender back
    ...
    # In order to be fast, the following maps are applied over the program and not on the database    
    sender_id = senders.get(sender, None)
    subject_id = subjects.get(subject, None)
    guid_id = guids.get(guid, None)
    
    # Put the senders into the database and into the dictionaries
    ...
    # Insert the message into de database accordingly. zlib.compress saves a compressed version of the message
    ...
    try:
        # Get the primary key and update the dictionary
        row = cur.fetchone()
        message_id = row[0]
